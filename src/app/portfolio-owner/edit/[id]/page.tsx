...
export default function EditPortfolio({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchPortfolio = async () => {
    try {
      ...
      if (!token) {
        console.log('No token found for edit');
        setMessage('Authentication required');
        setFetchLoading(false);
        return;
      }

      console.log('Fetching portfolio for edit:', portfolioId);
      const response = await fetch(`/api/portfolios/edit/${portfolioId}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        setImages(Array.isArray(portfolio.images) ? portfolio.images : []);
        setTagList(Array.isArray(portfolio.tags) ? portfolio.tags : []);
      } else {
        const errorText = await response.text();
        console.error('Edit fetch failed:', response.status, errorText);
        setMessage('Portfolio not found or unauthorized');
      }
    } catch (error) {
      console.error('Failed to fetch portfolio:', error);
      setMessage('Failed to fetch portfolio');
    } finally {
      setFetchLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(errorData.error || 'Failed to update portfolio');
      }
    } catch (error) {
